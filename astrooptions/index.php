...
<?php 
$email_v = 'false';
$email_v_status = 'false';

if(isset($_GET['vr']) && isset($_GET['pri']) && isset($_GET['email']) && isset($_GET['et'])){
    $get_email = $_GET['email'];
    $get_sr = $_GET['vr'];
    $get_pri = $_GET['pri'];
    $get_et = $_GET['et'];
 $sql = "SELECT * FROM user_verification WHERE email='$get_email' AND elapse_time='$get_et'";
        $result = mysqli_query($link, $sql);
        $count = mysqli_num_rows($result);
        if($count == 1){
            $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
            ...
            if(time() < $main_et && $get_sr == $main_sr && $main_count == 0){
               $email_v = 'true'; 
                ...
                `account_status`='Verified' WHERE `email` = '$get_email'";  
...
-->
                ...
                <div  id="em_verify_block" style="display:none">
                    <br>
                        <div class="alert alert-danger em_verify_block" style="border-radius:3px;text-align:center;background-color:#E23D28;color:#fff;padding:10px 85px;margin-top:0px">Your email is not verified. Click <span class="em_verify_button"><a href="" style="background-color:green;padding:4px;border-radius:2px" id="em_verify_button">Send</a></span> to send verification email
                        </div>
                    </div>
                <div class="account-modal-bottom l_message" style="padding: 0 !important; position: relative; margin: 40px auto !important;text-align: center;display: flex;justify-content: center; ">
                    ...
                    <?php
                    if($email_v == "true"){
                        if($email_v_status == "true"){
                            echo '<div class="alert alert-danger" style="border-radius:3px;text-align:center;background-color:green;color:#fff;padding:10px 85px;margin-top:0px"> E-mail Verification Successful</div>';
                        }else{
                            echo '<div class="alert alert-danger" style="border-radius:3px;text-align:center;background-color:#E23D28;color:#fff;padding:10px 85px;margin-top:0px"> E-mail Verification failed</div>';
                        }
                    }
                    ?>
...
//            console.log("Test Load");
                        $("#em_verify_button").click(function(event) {
                            event.preventDefault();
                            console.log("V CLicked");
                            var send_v_email = "send_v_email";
                            var email = $("#login_email").val();
                            if(email == ""){
                                $(".l_message").html('<div class="alert alert-danger" style="border-radius:3px;text-align:center;background-color:#E23D28;color:#fff;padding:10px 85px;margin-top:0px">Please Enter E-mail first in the field above.</div>');
                            }else{
                                $('#em_verify_block').css('display','none');
                                $.ajax({
                                    type: "POST",
                                    url: "phpscripts/send_verification_email.php/",
                                    data: {
                                        email: email,
                                        send_v_email: send_v_email
                                    },
...
//                console.log("email: " + email + " password: " + password );
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if(response == false){
                            $(".l_message").html("");
                            $('#em_verify_block').css('display','block');
                        }else{
                          $(".l_message").html(response);  
                        }
...
<!--              <h1 style="text-align:center;color:#A38A00">+$200 </h1>  -->
              ...
              <h5 style="text-align:center;">Deposit $200+ to your account within 24hrs, and enjoy extra benefit: <span style="color:#A38A00">$200 Bonus</span></h5><br> 
              <h5 style="text-align:center;">Or Deposit $100-$199 to your account within 24hrs, and enjoy extra benefit: <span style="color:#A38A00">$20 Bonus</span></h5>
                <br>
